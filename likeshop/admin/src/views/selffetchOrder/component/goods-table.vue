<template>
    <el-table size="large" :data="goods"  @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" :selectable="selected" v-if="showSelected==true" />
...
const selected = (row, index) => {
    if (row.afterSale != 1 || row.verificationStatus == 1) {
        return false
    } else {
        return true
    }
