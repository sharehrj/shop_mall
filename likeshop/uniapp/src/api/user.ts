...
export function apiAccountLogLists(data: any) {
    return request.get({ url: '/logs/userEarnings', data })
}
export function userLevelInfo(data?: any) {
    return request.get({ url: '/distribution/level', data })
}
//分销配置
export function distributionconfig(data?: any) {
    return request.get({ url: '/distribution/config', data })
}

// 注销账号
export function apiUserCancellation(data?: any) {
    return request.get({ url: '/user/cancellation', data })
}