...
//        }
        if (menuIds != null && !menuIds.equals("")) {
            List<SystemAuthPerm> permList = new ArrayList<>();
            String[] menuIdArray = menuIds.split(",");

            for (String menuId : menuIdArray) {
                SystemAuthPerm model = new SystemAuthPerm();
                model.setId(ToolUtils.makeUUID()); // 手动生成UUID
                model.setRoleId(roleId);
                model.setMenuId(Integer.parseInt(menuId.trim()));
                permList.add(model);
            }

            // 使用自定义批量插入方法
            if (!permList.isEmpty()) {
                systemAuthPermMapper.batchInsertPerms(permList);
            }
        }
    }
