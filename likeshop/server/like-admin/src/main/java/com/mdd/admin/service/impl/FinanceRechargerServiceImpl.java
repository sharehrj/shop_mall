...
public class FinanceRechargerServiceImpl implements IFinanceRechargerService {
    ...
    public PageResult<FinanceRechargeListVo> list(PageValidate pageValidate, FinanceRechargeSearchValidate searchValidate) {
        ...
        for (FinanceRechargeListVo vo : iPage.getRecords()) {
            ...
            vo.setPayTime(vo.getPayStatus().equals(1) ? TimeUtils.timestampToDate(vo.getPayTime()) : "-");
    ...
    public void refund(Integer orderId, Integer adminId) {
        ...
        try {
            ...
            // 发起退款请求
            switch (refundRecord.getRefundWay()){
                case 2:
                ...
                case 3:
                    ...
                    request.setTransactionId(rechargeOrder.getTransactionId());
                    request.setOutTradeNo(rechargeOrder.getOrderSn());
                    request.setOutRefundNo(refundSn);
                    request.setTotalAmount(rechargeOrder.getOrderAmount());
                    request.setRefundAmount(rechargeOrder.getOrderAmount());
    ...
    public void refundAgain(Integer recordId, Integer adminId) {
        ...
        try {
            ...
            switch (refundRecord.getRefundWay()){
                ...
                case 3:
                    AliPayRefundRequest request = new AliPayRefundRequest();
                    request.setTransactionId(refundRecord.getTransactionId());
                    request.setOutTradeNo(refundRecord.getOrderSn());
                    request.setOutRefundNo(refundRecord.getSn());
                    request.setTotalAmount(rechargeOrder.getOrderAmount());
                    request.setRefundAmount(rechargeOrder.getOrderAmount());
                    AliPayDriver.refund(request);
                    break;
