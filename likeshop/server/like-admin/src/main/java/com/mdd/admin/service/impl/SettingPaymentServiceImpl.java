...
public class SettingPaymentServiceImpl implements ISettingPaymentService {
    ...
    public DevPayConfig detail(Integer id) {
        ...
        params.put("mch_id", envStatus ? "******" : params.getOrDefault("mch_id", ""));
        params.put("pay_sign_key", envStatus ? "******" : params.getOrDefault("pay_sign_key", ""));
        params.put("private_cert", envStatus ? "******" : params.getOrDefault("private_cert", ""));
        params.put("private_key", envStatus ? "******" : params.getOrDefault("private_key", ""));

        devPayConfig.setName(PaymentEnum.getPayWayMsg(devPayConfig.getWay()));
        devPayConfig.setShowName(devPayConfig.getName());
        devPayConfig.setParams(params);
